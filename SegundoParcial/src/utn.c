...
static int myGets(char* cadena, int longitud)
{
	...
	{
		__fpurge(stdin); // LINUX-> __fpurge o WIN-> fflush o MAC-> fpurge
...
{
    int output = -1;
    if (mensaje != NULL && mensajeError != NULL && pCadena != NULL && limite > 0 && reintentos > 0)
    {
        do {
            printf("%s", mensaje);
